...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, timestamps_total, history_time_nogt, mode):
    ...
    history_len = len(history_list) - len(test_list)
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        history_gglist = []
        history_ggglist = []
        for ii, g in enumerate(input_list):
            if ii<len(input_list)-1:
                snap = np.concatenate((input_list[ii], input_list[ii+1]))
                gg = build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu)
                history_gglist.append(gg)
        ...
        if args.multi_step:
            ...
            all_tail_seq = sp.load_npz(
                '../data/{}/along/history/tail_history_{}.npz'.format(args.dataset, timestamps_total[history_len+time_idx-1]))
            seq_idx = histroy_data[:, 0] * num_rels * 2 + histroy_data[:, 1]

            tail_seq = torch.Tensor(all_tail_seq[seq_idx].todense())
            one_hot_tail_seq = tail_seq.masked_fill(tail_seq != 0, 1)

            rel_seq_begin = sp.load_npz(
            '../data/{}/along/history/rel_history_{}.npz'.format(args.dataset, timestamps_total[history_len+time_idx-args.test_history_len-1]))
            all_rel_seq = sp.load_npz(
                '../data/{}/along/history/rel_history_{}.npz'.format(args.dataset, timestamps_total[history_len+time_idx-1]))
            rel_seq_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]

            all_rel_seq = all_rel_seq - rel_seq_begin
            rel_seq = torch.Tensor(all_rel_seq[rel_seq_idx].todense())
            one_hot_rel_seq = rel_seq.masked_fill(rel_seq != 0, 1)
        if use_cuda:
        ...
        test_triples, final_score, final_r_score = model.predict(history_glist, history_gglist, history_ggglist, num_rels, static_graph, test_triples_input, one_hot_tail_seq, one_hot_rel_seq, use_cuda)
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    timestamps_total = np.append(train_times,valid_times,axis=0)
    timestamps_total = np.append(timestamps_total,test_times,axis=0)
    ...
    model_name = "{}_{}_rate_{}-{}-{}-ly{}-dilate{}-his{}-weight_{}-discount_{}-angle_{}-dp{}_{}_{}_{}-gpu{}-{}"\
        .format(args.idname, args.dataset, args.history_rate, args.encoder, args.decoder, args.n_layers, args.dilate_len, args.train_history_len, args.weight, args.discount, args.angle,
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                # pos_union_graph = build_union_graph(input_list, histroy_data, num_nodes, num_rels, use_cuda, args.gpu)
                inverse_histroy_data = histroy_data[:, [2, 1, 0, 3]]
                inverse_histroy_data[:, 1] = inverse_histroy_data[:, 1] + num_rels
                # inverse_union_graph = build_union_graph(input_list, inverse_histroy_data, num_nodes, num_rels, use_cuda, args.gpu)
                histroy_data = torch.cat([histroy_data, inverse_histroy_data])
                histroy_data = histroy_data.cpu().numpy()
                if train_sample_num == 0:
                    all_tail_seq = sp.csr_matrix(([], ([], [])), shape=(num_nodes * num_rels * 2, num_nodes))
                else:
                    all_tail_seq = sp.load_npz(
                        '../data/{}/along/history/tail_history_{}.npz'.format(args.dataset, train_times[train_sample_num-1]))
                seq_idx = histroy_data[:, 0] * num_rels * 2 + histroy_data[:, 1]

                tail_seq = torch.Tensor(all_tail_seq[seq_idx].todense())
                one_hot_tail_seq = tail_seq.masked_fill(tail_seq != 0, 1)
                if train_sample_num - args.train_history_len<=0:
                    rel_seq_begin = sp.csr_matrix(([], ([], [])), shape=(num_nodes * num_nodes, num_rels * 2))
                else:
                    rel_seq_begin = sp.load_npz(
                    '../data/{}/along/history/rel_history_{}.npz'.format(args.dataset, train_times[train_sample_num-args.train_history_len-1]))

                if train_sample_num == 0:
                    all_rel_seq = sp.csr_matrix(([], ([], [])), shape=(num_nodes * num_nodes, num_rels * 2))
                else:
                    all_rel_seq = sp.load_npz(
                    '../data/{}/along/history/rel_history_{}.npz'.format(args.dataset, train_times[train_sample_num-1]))
                rel_seq_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]
                ...
                loss_e, loss_r, loss_static, loss_spc = model.get_loss(history_glist, history_gglist, history_ggglist, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, use_cuda)
                loss = args.task_weight*loss_e + (1-args.task_weight)*loss_r + loss_static# + loss_spc

                losses.append(loss.item())
                losses_e.append(loss_e.item())
                losses_r.append(loss_r.item())
                losses_spc.append(loss_spc.item())
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='HisRES')
    ...
    parser.add_argument("--timestamps", type=int, default=365,
                        help="total timestamps")
    ...
    parser.add_argument("--idname", type=str, default="ICEWS14",
                        help="the idname for command")

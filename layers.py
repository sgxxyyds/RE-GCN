...
class CandRGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1,  bias=None,
        ...
        self.pos_proj = nn.Linear(3*in_feat, 3*out_feat, bias=False)
        ...
        channels = 1
        kernel_size = 1
        self.conv = torch.nn.Conv1d(1, channels, kernel_size, stride=1,
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        if self.activation:
            ...
            # node_repr = self.swish(node_repr)
    ...
    def edge_attention(self, edges):
        z2 = torch.cat([edges.src['h'], edges.data['r'], edges.dst['h']], dim=1)
        a = self.pos_proj(z2)
...
class UnionRGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1,  bias=None,
        ...
        nn.init.xavier_uniform_(self.weight_neighbor, gain=nn.init.calculate_gain('relu'))
        # WR
        self.r_linear = nn.Linear(self.in_feat, self.out_feat)
        #* add batchnorm
        # self.batchnorm = nn.BatchNorm1d(self.in_feat)
        #* add swish
        # self.swish_linear = nn.Linear(in_feat, in_feat)
    ...
    # def swish(self, x):
    #     return x*torch.sigmoid(self.swish_linear(x))
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        # self.rel_emb = self.r_linear(emb_rel)
        # self.sub = sub
        # self.ob = ob
        if self.self_loop:
            masked_index = torch.masked_select(
        ...
        self.propagate(g)
        node_repr = g.ndata['h']
        # edge_repr = self.batchnorm(self.r_linear(self.rel_emb))
        edge_repr = self.r_linear(self.rel_emb)
        # print(len(prev_h))
        if len(prev_h) != 0 and self.skip_connect:  # 两次计算loop_message的方式不一样，前者激活后再加权
            if self.self_loop:
                node_repr = node_repr + loop_message
        ...
        if self.activation:
            node_repr = self.activation(node_repr)
            edge_repr = self.activation(edge_repr)
            # node_repr = self.swish(node_repr)
            # edge_repr = self.swish(edge_repr)
            # self.rel_emb = self.activation(self.rel_emb)
        if self.dropout is not None:
            node_repr = self.dropout(node_repr)
            edge_repr = self.dropout(edge_repr)
            # self.rel_emb = self.dropout(self.rel_emb)
        g.ndata['h'] = node_repr

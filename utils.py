...
# def build_union_graph(input_list, num_nodes, num_rels, use_cuda, gpu):
#     def comp_deg_norm(g):
#         in_deg = g.in_degrees(range(g.number_of_nodes())).float()
#         in_deg[torch.nonzero(in_deg == 0).view(-1)] = 1
#         norm = 1.0 / in_deg
...
#     for id in range(0, input_len):
#         triples = input_list[id][:, :3]
#         time_id = input_list[id][:, 3]
#         if id == 0:
#             src, rel, dst = triples.transpose()
#             src, dst = np.concatenate((src, dst)), np.concatenate((dst, src))
#             rel = np.concatenate((rel, rel + num_rels))
#             tt_list = np.concatenate((time_id, time_id))
#             # print(len(rel))
#             # print(len(tt_list))
#         else:
#             i_src, i_rel, i_dst = triples.transpose()
#             i_src, i_dst = np.concatenate((i_src, i_dst)), np.concatenate((i_dst, i_src))
#             i_rel = np.concatenate((i_rel, i_rel + num_rels))
#             src, dst = np.concatenate((src, i_src)), np.concatenate((dst, i_dst))
#             rel = np.concatenate((rel, i_rel))
#             tt_list = np.concatenate((tt_list, time_id, time_id))
#     g = dgl.DGLGraph()
#     g.add_nodes(num_nodes)
#     g.add_edges(src, dst)
#     norm = comp_deg_norm(g)
...
#     g.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})
#     g.edata['type'] = torch.LongTensor(rel)
#     g.edata['time_id'] = torch.LongTensor(tt_list)
...
#* 22222
def build_his_graph(num_nodes, num_rels, total_triples, voc, gpu, inv=False):
    # if pred_sub:
    ...
    #     rel = torch.repeat_interleave(total_rel[indices], k)
    #     time = torch.repeat_interleave(time_id[indices], k)
    #     obj = topk_obj.view(-1)
    #     graph.add_edges(sub, obj)
    ...
    #     )
    #     norm = comp_deg_norm(graph)
    #     graph.ndata["id"] = torch.arange(num_nodes, device=graph.device)
    #     graph.ndata['norm'] = norm.view(-1, 1)
    #     graph.edata['type'] = rel
    #     graph.edata['time_id'] = time
    #     graph.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})
    #     graph_list.append(graph)
        # graph.to(gpu)
    #* 确保graph的dense
    # if torch.mean(graph.in_degrees().float()) < 0.5:


    norm = comp_deg_norm(graph)
    graph.ndata["id"] = torch.arange(num_nodes, device=graph.device)
    graph.ndata['norm'] = norm.view(-1, 1)
    graph.edata['type'] = edge
    # graph.edata['time_id'] = t
    graph.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})
    # print(graph.in_degrees().shape)
    # print(torch.mean(graph.in_degrees().float()))
    # print(graph.num_edges()/(num_nodes*(num_nodes-1)))
    # print(graph.num_edges())
    return graph

#* 11111
# def build_his_graph(num_nodes, total_triples, total_obj_logit, k, num_partitions, voc, gpu):

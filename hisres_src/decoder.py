...
class TimeConvTransE(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rel, time_embs, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        e1_embedded_all = F.tanh(embedding)
        batch_size = len(triplets)
        ...
        #* add time_embs
        # stacked_inputs = stacked_inputs + time_embs.unsqueeze(1)
        #* ab4
        # stacked_inputs = torch.cat([e1_embedded, rel_embedded], 1)  # batch_size,2,h_dim
